...
# _buckets = [(5000, 200),(10000, 200),(15000, 200),(20000, 200),(25000, 200)]
_buckets = [(1500, 200)]
...
def decode():
  with tf.Session() as sess:
	...
	with open("data/test.enc") as f:
		sentences = map(lambda x: x.strip(), f.readlines())[:50]
	output_sentences = []
	with open("data/test.dec") as f:
		actual_headlines = map(lambda x: x.strip(), f.readlines())[:50]
	output_sentences = []
	for index, sentence in enumerate(sentences):
	  ...
	  output_sentences.append("actual:" + actual_headlines[index])
	  output_sentences.append("generated:" + " ".join([tf.compat.as_str(rev_dec_vocab[output]) for output in outputs]))
...
def decode_line(sess, model, enc_vocab, rev_dec_vocab, sentence):
	...
	# This is a greedy decoder - outputs are just argmaxes of output_logits.
	outputs = [int(np.argmin(logit, axis=1)) for logit in output_logits]
